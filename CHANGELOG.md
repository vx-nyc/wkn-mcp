...
- OpenClaw installs now apply the `group:plugins` tool policy and compact
  `toolSearch` surface required for local live VX MCP agent turns, avoiding
  unknown-tool warnings and immediate prompt compaction failures.

## [1.0.8] - 2026-06-11

### Added

- `vx-mcp login openclaw` starts the OpenClaw OAuth flow through the installed
  OpenClaw CLI or `npx openclaw` when a VX MCP config is present.
- `vx-mcp login all` runs every supported CLI login helper in one pass.
